...
if (!process.env.JWT_SECRET) {
  console.warn('WARNING: JWT_SECRET not set. Using a random secret — existing sessions will not survive server restarts. Set JWT_SECRET in .env.');
}
...
// Watsonx Token Generation for Embedded Chat Security
app.get('/api/watsonx/token', auth, async (req, res) => {
  try {
    ...
    // Sign the token using the secret from environment
    const WXO_INSTANCE_SECRET = process.env.WATSONX_INSTANCE_SECRET;
    if (!WXO_INSTANCE_SECRET) {
      console.error('WATSONX_INSTANCE_SECRET is missing in .env');
    ...
    const token = jwt.sign(payload, WXO_INSTANCE_SECRET, { algorithm: 'HS256' });
    res.json({ token });
  } catch (err) {
    console.error('Watsonx token error:', err);
