...
async function performSemanticKeywordResearch(baseKeyword, competitorData) {
  try {
    ...
    // AI Enhancement: Attempt to get keyword suggestions from Watsonx agent
    try {
      const aiKeywords = await agentService.generateAIKeywords(baseKeyword);
      if (aiKeywords && Array.isArray(aiKeywords) && aiKeywords.length > 0) {
      ...
      console.warn('AI keyword research skipped or failed:', aiError.message);
    }
