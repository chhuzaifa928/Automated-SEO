...
async function performSEOAudit(url, htmlContent) {
  try {
    ...
    try {
      ...
      // AI Enhancement: Attempt to get a more sophisticated analysis from Watsonx agent
      try {
        const aiAudit = await agentService.generateAISAudit(url || 'HTML Content', {
        ...
        if (aiAudit && aiAudit.score !== undefined) {
          console.log('AI Audit successful, merging results...');
          // Merge AI results with rule-based results
          // We keep the technical elements and findings but can use AI for score and high-level assessment
          auditResult.score = aiAudit.score;
          auditResult.grade = aiAudit.grade || auditResult.grade;
          if (aiAudit.issues && aiAudit.issues.length > 0) {
            // Prepend AI issues if they are distinct
            auditResult.issues = [...aiAudit.issues.map(i => ({ ...i, source: 'AI' })), ...auditResult.issues];
          }
          if (aiAudit.recommendations && aiAudit.recommendations.length > 0) {
            auditResult.recommendations = [...aiAudit.recommendations, ...auditResult.recommendations];
          }
        }
      } catch (aiError) {
        console.warn('AI analysis skipped or failed:', aiError.message);
      }
    } catch (checkError) {
